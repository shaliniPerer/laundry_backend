...
reportsRouter.get("/profit-loss", async (req, res) => {
    ...
    const sales = salesRows.filter((s) => s.entityType === "SALE" && (!from || s.deliveryDate >= from) && (!to || s.deliveryDate <= to));
    const returns = retRows.filter((r) => r.entityType === "SALE_RETURN" && (!from || (r.createdAt?.slice(0, 10) ?? "") >= from) && (!to || (r.createdAt?.slice(0, 10) ?? "") <= to));
    const expenses = expRows.filter((e) => e.entityType === "EXPENSE" && (!from || e.date >= from) && (!to || e.date <= to));
    const items = itemRows.filter((i) => i.entityType === "ITEM");
    ...
    // Item-wise profit
    const itemMap = Object.fromEntries(items.map((i) => [i.pk.replace("ITEM#", ""), i]));
    const itemWiseMap = {};
