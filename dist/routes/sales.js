...
salesRouter.post("/", async (req, res) => {
    ...
    if (req.userId) {
        const userRec = await getItem(`USER#${req.userId}`, "PROFILE");
        if (userRec?.name)
            createdBy = userRec.name;
    }
...
salesRouter.post("/pos", async (req, res) => {
    ...
    const rec = {
        ...
        otherChargeItems: Array.isArray(body.otherChargeItems) ? body.otherChargeItems : undefined,
...
salesRouter.post("/:id/payments", async (req, res) => {
    const existing = await getItem(`SALE#${req.params.id}`, "META");
    if (!existing || existing.entityType !== "SALE") {
        res.status(404).json({ error: "Not found" });
        return;
    }
    const { amount, paymentType, date, note } = req.body;
...
salesRouter.patch("/:id/payments/:paymentId", async (req, res) => {
    ...
    const { amount, paymentType, date, note } = req.body;
    const payments = (existing.payments ?? []).map((p) => p.id === req.params.paymentId
        ? {
            ...p,
...
salesRouter.put("/:id", async (req, res) => {
    ...
    const updates = {
        ...
        otherChargeItems: Array.isArray(body.otherChargeItems) ? body.otherChargeItems : existing.otherChargeItems,
        discountOnAll,
...
salesRouter.patch("/:id/status", async (req, res) => {
    ...
    notificationManager.onJobStatusChange(status, customerPhone, {
