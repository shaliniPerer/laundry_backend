...
export async function scanByEntityPrefix(prefix) {
...
export async function scanSalesByDeliveryRange(start, end) {
    const res = await docClient.send(new ScanCommand({
        TableName,
...
export async function updateItem(pk, sk, updates) {
    ...
    for (const [k, v] of Object.entries(updates)) {
        if (v === undefined)
            continue; // skip undefined — leave existing DynamoDB value unchanged
        const nk = `#n${i}`;
        const vk = `:v${i}`;
        names[nk] = k;
        values[vk] = v;
