...
import type { ExpenseCategoryRecord, ExpenseRecord } from "../types.js";
...
async function saveExpenseAttachment(expenseId: string, attachment?: IncomingAttachment) {
  if (!attachment?.dataUrl || !attachment.fileName || !attachment.mimeType) return undefined;
...
expensesRouter.get("/", async (_req, res) => {
  ...
  const list = rows.filter((r) => (r as { entityType?: string }).entityType === "EXPENSE");
  res.json({ expenses: list });
});

expensesRouter.get("/categories/list", async (_req, res) => {
  const rows = (await scanByEntityPrefix("EXPENSE_CAT#")) as ExpenseCategoryRecord[];
...
expensesRouter.patch("/categories/:id", async (req, res) => {
  ...
  const { name, description, status } = req.body as Partial<ExpenseCategoryRecord>;
...
expensesRouter.patch("/:id", async (req, res) => {
  ...
  const b = req.body as Partial<ExpenseRecord>;
