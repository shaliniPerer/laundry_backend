...
suppliersRouter.post("/", async (req, res) => {
  const b = req.body as Partial<SupplierRecord>;
  if (!b.name) { res.status(400).json({ error: "name required" }); return; }
  const existing = await scanByEntityPrefix("SUPPLIER#");
  const count = existing.filter((r) => (r as { entityType?: string }).entityType === "SUPPLIER").length + 1;
...
suppliersRouter.patch("/:id", async (req, res) => {
  ...
  const allowed: (keyof SupplierRecord)[] = ["name", "mobile", "phone", "email", "gstNumber", "taxNumber", "country", "state", "city", "postcode", "address", "previousDue", "status"];
...
suppliersRouter.post("/import", async (req, res) => {
  const { rows } = req.body as { rows: Partial<SupplierRecord>[] };
  if (!Array.isArray(rows)) { res.status(400).json({ error: "rows required" }); return; }
  const existing = await scanByEntityPrefix("SUPPLIER#");
  let count = existing.filter((r) => (r as { entityType?: string }).entityType === "SUPPLIER").length;
  let imported = 0;
  for (const b of rows) {
    if (!b.name) continue;
    count++;
