...
import type { PurchaseRecord, PurchaseReturnRecord } from "../types.js";
...
purchasesRouter.post("/", async (req, res) => {
  const b = req.body as Partial<PurchaseRecord> & { createdBy?: string };
  const existing = await scanByEntityPrefix("PURCHASE#");
  const count = existing.filter((r) => (r as { entityType?: string }).entityType === "PURCHASE").length + 1;
...
purchasesRouter.patch("/:id", async (req, res) => {
  ...
  const b = req.body as Partial<PurchaseRecord>;
  const updates: Record<string, unknown> = { updatedAt: now() };
  const allowed: (keyof PurchaseRecord)[] = ["supplierId", "supplierName", "purchaseDate", "status", "referenceNo", "subtotal", "otherCharges", "otherChargesType", "discountOnAll", "discountOnAllType", "roundOff", "total", "note", "paymentStatus", "createdBy", "lines"];
...
purchasesRouter.post("/returns", async (req, res) => {
  const b = req.body as Partial<PurchaseReturnRecord>;
  const existing = await scanByEntityPrefix("PURCHASE_RETURN#");
  const count = existing.filter((r) => (r as { entityType?: string }).entityType === "PURCHASE_RETURN").length + 1;
  ...
  const roundOff = b.roundOff != null ? Number(b.roundOff) : 0;
  const total = subtotal + otherCharges - discountOnAll + roundOff;
  const rec: PurchaseReturnRecord = {
...
purchasesRouter.patch("/returns/:id", async (req, res) => {
  ...
  const b = req.body as Partial<PurchaseReturnRecord>;
  const updates: Record<string, unknown> = { updatedAt: now() };
  const allowed: (keyof PurchaseReturnRecord)[] = ["supplierId", "supplierName", "date", "status", "referenceNo", "subtotal", "otherCharges", "discountOnAll", "roundOff", "total", "note", "paymentStatus", "lines"];
  for (const k of allowed) { if (b[k] !== undefined) updates[k] = b[k]; }
