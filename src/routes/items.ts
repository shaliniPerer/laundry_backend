...
import type { BrandRecord, CategoryRecord, ItemRecord } from "../types.js";
...
itemsRouter.post("/", async (req, res) => {
  const body = req.body as Partial<ItemRecord>;
  if (!body.name) {
    res.status(400).json({ error: "name required" });
    return;
  }
  const existing = await scanByEntityPrefix("ITEM#");
  const count = existing.filter((r) => (r as { entityType?: string }).entityType === "ITEM").length + 1;
...
itemsRouter.post("/import", async (req, res) => {
  const { rows } = req.body as { rows?: Partial<ItemRecord>[] };
...
itemsRouter.get("/categories", async (req, res) => {
  const kind = (req.query.kind as string) || "item";
  const rows = (await scanByEntityPrefix("CATEGORY#")) as CategoryRecord[];
  res.json({ categories: rows.filter((c) => !kind || c.kind === kind) });
});

itemsRouter.post("/categories", async (req, res) => {
  const { name, kind, description } = req.body as { name?: string; kind?: "item" | "expense"; description?: string };
  if (!name || !kind) {
    res.status(400).json({ error: "name and kind required" });
    return;
  }
  const existing = await scanByEntityPrefix("CATEGORY#");
  const count = existing.filter((r) => (r as { entityType?: string }).entityType === "CATEGORY").length + 1;
...
itemsRouter.patch("/categories/:id", async (req, res) => {
  ...
  const b = req.body as Partial<CategoryRecord>;
  await updateItem(row.pk as string, row.sk as string, {
...
itemsRouter.delete("/categories/:id", async (req, res) => {
  ...
  await deleteItem(row.pk as string, row.sk as string);
  res.json({ ok: true });
});
...
itemsRouter.post("/brands", async (req, res) => {
  ...
  const count = existing.filter((r) => (r as { entityType?: string }).entityType === "BRAND").length + 1;
...
itemsRouter.patch("/brands/:id", async (req, res) => {
  ...
  const b = req.body as Partial<BrandRecord>;
...
itemsRouter.patch("/:id", async (req, res) => {
  ...
  const b = req.body as Partial<ItemRecord>;
  await updateItem(pk, "PROFILE", {
    ...(b.name != null ? { name: b.name } : {}),
