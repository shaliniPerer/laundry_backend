...
import { renderTemplate, type SmsTemplate, type TemplateData } from "./templateEngine.js";
import { TEMPLATE_REGISTRY, type EventType } from "./templateRegistry.js";
import { config } from "../config.js";
import { getItem } from "../db/repo.js";
import type { SmsTemplateRecord } from "../types.js";

// ── Job Status → EventType Map ────────────────────────────────────────────────
export const JOB_STATUS_EVENT_MAP: Readonly<Record<string, EventType>> = {
  Placed:              "JOB_PLACED",
  Done:                "JOB_COMPLETED",
...
class NotificationManager {
  ...
  async fire(
    ...
    const dbRecord = await getItem(`SMSTEMPLATE#${templateId}`, "META") as SmsTemplateRecord | undefined;

    // 2. Fall back to static registry for built-in templates
    const systemTemplate = TEMPLATE_REGISTRY[templateId as EventType];

    let effectiveTemplate: SmsTemplate;
    if (dbRecord?.body) {
      effectiveTemplate = {
        id:          templateId,
    ...
    if (!result.success) {
      console.error(
        `[NotificationManager] ${templateId} → ${phone} FAILED: ${result.error}`
      );
    }
  }
